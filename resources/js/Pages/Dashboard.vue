...
<!--            <DriverList1 :routeFunction="showroute" />-->
              ...
              <div class="flex p-4 cursor-pointer" v-for="(location, index) in locations" v-if="locations.length>0&&location.title.toLowerCase()!=='ivan tracker'" @click="selectedMarker = index; centerMapToPosition(location.positionData.latitude,location.positionData.longitude)">
                  ...
                  <div>
                      ...
                      <small>{{location.positionData["attributes"]["batteryLevel"]}}{{ location.positionData["attributes"]["batteryLevel"]?'%':'' }}</small>
                  </div>
              </div>
          </div>
        </aside>
