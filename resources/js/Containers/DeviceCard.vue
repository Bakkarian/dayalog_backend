...
import { XMarkIcon  } from '@heroicons/vue/24/outline'
    ...
    const { buildLocationFromDevice, tracarPositions, centerMapToDevice } = useDashboardMap()
    ...
    watch(() => props.value.device, (newDevice) => {
        if (history.value == 0) {
            centerMapToDevice(newDevice.id)
        }
    })
    ...
    onMounted(()=>{
        ...
        if (history.value == 0) {
            // debugger
            centerMapToDevice(props.value.device.id)
        }
    })
